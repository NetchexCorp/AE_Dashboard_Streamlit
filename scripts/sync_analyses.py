#!/usr/bin/env python3
"""Sync RIaaS analysis templates between Azure Table Storage and analyses_snapshot.json.

Modes:
  --export   Table -> analyses_snapshot.json (run before git push / deploy)
  --import   analyses_snapshot.json -> Table (disaster recovery)
...
SNAPSHOT_PATH = REPO_ROOT / "analyses_snapshot.json"
TABLE_NAME = "RiAnalyses"
PARTITION = "analysis"
...
def do_import() -> int:
    ...
    client = _table_client()
    n_upsert = 0
    for analysis_id, template in snapshot.items():
        if table.get(analysis_id) == template:
            continue
...
def do_check() -> int:
    ...
    changes = _diff(snapshot, table)
    if changes:
        print("ERROR: analyses_snapshot.json is out of sync with Table Storage:")
        print("\n".join(changes))
        print("\nRun: python scripts/sync_analyses.py --export && git add analyses_snapshot.json")
        return 1
    return 0
